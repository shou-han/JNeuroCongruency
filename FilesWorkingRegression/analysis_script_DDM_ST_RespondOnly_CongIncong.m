...
%         %%%selection:
%         N2cselect = min(erpN2c(t>100 & t<600));
%         if N2cselect<0
%             allselect(trial)=1;
%         else
%             allselect(trial)=1;
        ...
        for d=1:2 %motion and colour
            for cong=1:2
                if d==1 %motion
                    ...
                    window_searchc = [278 388]; %JC: 324-424, S: 128-560
                    window_searchpc = [200 500];
                    window_searchCPP = [-200 -5];
                elseif d==2 %colour
                    window_searchi = [300 500];
                    window_searchc = [278 388];
...
for c=2
    for d=1:2
        ...
        if d==1 %motion
            window_searchi = [300 500];
            window_searchc = [321 371]; %JC: 324-424, S: 294-394, 150-600 works
            window_searchpc = [200 500];
            window_searchCPP = [-250 -5];
        elseif d==2 %colour
            window_searchi = [300 500];
            window_searchc = [321 371];
            window_searchpc = [150 300];
            window_searchCPP = [-200 -5];
        end
            ...
            for cong=1:2
                if ~isempty(conds{s,c,d,cong,bin})
                    ...
                    for jj=1:size(conds{s,c,d,cong,bin},2)                   
                        ...
                        [mediationsS.N2cpeak(k)] = min(N2cTemp(t>window_searchc(1) & t<window_searchc(2)));
                        [mediationsS.N2ipeak(k)] = min(N2iTemp(t>window_searchi(1) & t<window_searchi(2)));%200 to accomodate for N2c
                        [mediationsS.N2pcpeak(k)] = min(N2pcTemp(t>window_searchpc(1) & t<window_searchpc(2)));
...
if CSD
    save(['Data/ERPs/group_plots_erp_DDM_bin_ST_dual8Hz_CSD_N2cSelect321371'  num2str(single_participants) '_' num2str(chP)],'RTs','t','plot_chans','tr','chanlocs',...
    ...
    save(['Data/ERPs/group_plots_erp_DDM_bin_ST_dual8Hz_N2cSelect321371'  num2str(single_participants) '_' num2str(chP)],'RTs','t','plot_chans','tr','chanlocs',...
        'allBins','allsubj','subject_folder', 'allstuff','mediations','RT_side',...
        'conds','condsT','mediationsS','CPPAll','CPPrAll','RTAll','stimAll','congAll','N2cAll','N2iAll','responseAll');
end
%Plot_original