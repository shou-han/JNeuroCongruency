...
for s=1:size(allsubj,2)
    ...
    sT = allsubj(s)
            subjIndx = subjCount;
    if CSD
        load(['Data/ERPs/group_plots_erp_DDM_bin_ST_dual8Hz_CSD_N2cSelect321371' num2str(sT) '_' num2str(chn)],'mediations','mediationsS','chanlocs','t','tr','subject_folder')
        ...
        load(['Data/ERPs/group_plots_erp_DDM_bin_ST_dual8Hz_N2cSelect321371' num2str(sT) '_' num2str(chn)],'mediations','mediationsS','chanlocs','t','tr','subject_folder')
...
%             mediationsS.Betatrial(indxB) = tempzz;
    ...
    for stim=1:2
        for cong=1:2
            for run=2
                [blah,indx]=find(mediationsS.stim==stim & mediationsS.cong == cong & ismember(mediationsS.run,runs(run,:))); %color or motion
...
[indx, ~] = find(mediationAll.run>=2  & mediationAll.stim==1  & mediationAll.response ==1);
